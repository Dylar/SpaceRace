...
class AsteroidGroup(offsetX: Float = 0f, offsetY: Float = 0f) : SpaceGroup(offsetX, offsetY) {
    ...
    init {
        ...
        //BOTTOM
        val centerBottomField = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        addField(centerBottomField, SCREEN_WIDTH / 2)
        val leftBottomCorner = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        addField(leftBottomCorner, centerBottomField, -FIELD_PADDING_XXLARGE, connection = ConnectionPoint.BOTTOM)
        val rightBottomCorner = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        ...
        //TOP
        val centerTopField = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        addField(centerTopField, centerBottomField, verticalMod = FIELD_PADDING_XXLARGE)
        val leftTopCorner = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        addField(leftTopCorner, centerTopField, -FIELD_PADDING_XXLARGE, connection = ConnectionPoint.TOP)
        val rightTopCorner = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        ...
        //CENTER
        val leftCenterField = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
        addField(leftCenterField, centerBottomField, -FIELD_PADDING_LARGE, FIELD_PADDING_LARGE, ConnectionPoint.LEFT)
        val rightCenterField = FieldGraphic.createFieldOLD(FieldType.AMBUSH)
